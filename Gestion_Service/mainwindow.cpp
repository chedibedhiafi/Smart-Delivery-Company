     ...
     ui(new Ui::MainWindow)
{
    ...
    click->setMedia(QUrl::fromLocalFile("C:/Users/ASUS/Desktop/1/Gestion_Service/Click.mp3"));
...
//historique
    connect(ui->pushButtonHistorique, SIGNAL(clicked(bool)), this, SLOT(on_pushButtonHistorique_clicked()));
  /****************************************************************************************************************/
...
void MainWindow::on_pb_ajouter_an_clicked(){
  ...
  if(test){
    ...
    QFile file("C:/Users/ASUS/Desktop/1/Gestion_Service/Historique/HistoriqueA.txt");
    if(!file.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
        return;
    QTextStream cout(&file);
    QString codean_string= QString::number(codean);
    QString message2="\n Annonce a ete ajoutee sous le code "+codean_string+" avec le nom "+noman+" a la date "+datean+" avec la disc "+descriptionan+"";
    cout << message2;
ui->tab_annonce->setModel(tmpannonce.afficher_an());   //refresh
...
void MainWindow::on_pb_supprimer_an_clicked(){
    ...
    if(test){
        ...
        foreach(QLineEdit* le, findChildren<QLineEdit*>()) {
             le->clear();}
        QFile file("C:/Users/ASUS/Desktop/Gestion_Service/Historique/HistoriqueA.txt");
        if(!file.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
            return;
        QTextStream cout(&file);
        QString codean_string= QString::number(codean);
        QString message2="\n Annonce a ete supprimee sous le code"+codean_string+"";
        cout << message2;
...
void MainWindow::on_pb_modifier_an_clicked(){
    ...
    ui->codean_2->setValidator(new QIntValidator(0,999,this));
      //ui->codean_2->setValidator(validator_int);
        ...
        Annonce a(codean,noman, descriptionan,datean);
        bool test=a.modifier_annonce(codean);
        ...
        if(test ){
            ...
            QTextStream cout(&file);
            QString codean_string= QString::number(codean);
            QString message2="\n Annonce a ete modifiee sous le code"+codean_string+"avec le nom"+noman+"a la date"+datean+"avec la disc"+descriptionan+"";
            cout << message2;
ui->tab_annonce->setModel(tmpannonce.afficher_an());
